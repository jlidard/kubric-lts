...
parser = SceneGenerationArgumentParser()
parser.set_defaults(
    frame_end=1,
    resolution=(512, 512),
)
...
scene_filename = FLAGS.scene_filename

# parse the scene file to get the objects
...
# from utils import get_3x4_P_matrix_from_blender
# P, K, RT = get_3x4_P_matrix_from_blender(cam)
# logging.info(f"Camera intrinsics: {K}")
logging.info(f"Scene instruction: {instruction}")
logging.info(f"Scene geometry: {geometry}")
logging.info(f"Scene objects: {objects}")
...
for frame in range(1):
    ...
    categories = [obj for obj in all_categories if any(key.startswith(obj) for key in geometry.keys())]
    nonnative_categories = [obj for obj in all_nonnative_categories if any(key.startswith(obj) for key in geometry.keys())]
